...
import services.xenlan.hub.tablist.utils.MathUtils;
...
public enum TabColumn {
    ...
    public static TabColumn getFromSlot(Player player, Integer slot) {
        ...
        if (PlayerUtil.getPlayerVersion(player) == PlayerVersion.v1_7) {
            ...
            /* Left Column */
            if (MathUtils.isBetween(slot, 1, 20)) return LEFT;
            /* Middle Column */
            if (MathUtils.isBetween(slot, 21, 40)) return MIDDLE;
            /* Right Column */
            if (MathUtils.isBetween(slot, 41, 60)) return RIGHT;
            /* Far Right Column */
            if (MathUtils.isBetween(slot, 61, 80)) return FAR_RIGHT;
            return null;
        }
    }
